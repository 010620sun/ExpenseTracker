...
export const userStates = sqliteTable(
  "user_states",
  {
    ...
    baseCurrencyConfiguredAtMs: integer("base_currency_configured_at_ms"),
  ...
  (table) => [
    ...
    check(
      "user_states_base_currency_configured_at_positive",
      sql`${table.baseCurrencyConfiguredAtMs} IS NULL OR ${table.baseCurrencyConfiguredAtMs} > 0`,
    ),
    check(
...
export const monthlyBudgets = sqliteTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.category} IN ('housing', 'utilities', 'communication', 'groceries', 'dining', 'transport', 'vehicle', 'travel', 'health', 'personal_care', 'education', 'shopping', 'entertainment', 'subscriptions', 'family', 'pets', 'gifts', 'insurance', 'taxes', 'financial', 'other')`,
    ),
    check(
